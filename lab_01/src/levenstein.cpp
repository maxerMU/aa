...
size_t rec_cache(const string &str1, const size_t len1, const string &str2,
    ...
    if (mtr[len1][len2] != 0) {
        ...
        bool flag = str1[len1 - 1] != str2[len2 - 1];
        mtr[len1][len2] = min(min(rec_cache(str1, len1 - 1, str2, len2, mtr) + 1,
...
size_t Domerau(const string &str1, const size_t len1, const string &str2,
    ...
    if (mtr[len1][len2] != 0) {
        ...
        if (len1 > 1 && len2 > 1 && str1[len1 - 2] == str2[len2 - 1] && str1[len1 - 1] == str2[len2 - 2]) {
            mtr[len1][len2] = min(Domerau(str1, len1 - 2, str2, len2 - 2, mtr) + 1, mtr[len1][len2]);
        }
        else
        {
            bool flag = str1[len1 - 1] != str2[len2 - 1];
            mtr[len1][len2] = min(min(Domerau(str1, len1 - 1, str2, len2, mtr) + 1,
